  ...
  switch (command) {
    case "announce": //announces message specified in speified channel
      ...
      if (!message.guild.channels.find("name", channelid)) {
        //if specified channel does not exist or one is not specified, throws error
        return message.reply(
          "looks like you did't specify a channel or you spelled it wrong, try again."
        );
      }
      if (text == "") {
        //if no text is given, throws error
    ...
    case "kick": //kicks user, citing the reason given by the kicker
      ...
      const jrmodRole = message.guild.roles.find(
        "name",
      ...
      if (kickMember.roles.has(modRole.id || botRole.id || jrmodROle.id)) {
        //blocks kicking of a mod
        message.reply("You can't kick a mod, fool.");
      }
      if (!modRole) {
        //checks if there is a mod role in the server
        return console.log("The Mods role does not exist");
      }
      if (!message.member.roles.has(modRole.id)) {
        //checks if command user is mod
        return message.reply("You can't use this command.");
      }
      if (reason == "") {
        //if no reason is supplied it asks for one
        return message.reply("For what reason? Try again.");
      }

      kickMember.kick(reason).then((member) => {
        //kicks user and gives reason why
        message.channel.send(
          `${kickMember} was succesfully kicked for "${reason}"`
        );
      });
      break;
    ...
    case "clear": //clears a specified amount of messages from chat
      ...
      message.channel.bulkDelete(actualNumber).then(() => {
        //does the deleting
        message.channel
          .send(`Cleared ${args[0]} messages. ${swablu}`)
          .then((msg) => msg.delete(5000)); //sends conformation, then deletes conformation
      });
      break;

    case "junior": //gives junior role to sender        message.delete();
      if (
        message.member.roles.has("472534724032724992") ||
        message.member.roles.has("472534767024340993") ||
        message.member.roles.has("472537891776626689")
      ) {
        message.channel.send(
          `Aren\'t you already in a group? Contact a mod if you need a change! ${swablu}`
        );
    ...
    case "senior": //gives senior role to sender
      ...
      if (
        message.member.roles.has("472534683205500936") ||
        message.member.roles.has("472534767024340993") ||
        message.member.roles.has("472537891776626689")
      ) {
        message.channel.send(
    ...
    case "rotation": //sends the current rotation
      message.channel.send(
        "The 2020 Standard Rotation is Ultra Prism - Current. \nFind the current TCG Standard Rotation here : https://bit.ly/2UlfT4g"
      );
    ...
    case "location":
      message.channel.send(
        "We meet most Saturdays at Al's Sports Cards & Gaming.\n\nHere's the address!\n16 E 15th St, Edmond, OK 73013"
      );
    ...
    case "playerid":
      message.channel.send(
        "Find or create a player id here : http://bit.ly/2v9RIve"
      );
    ...
    case "facebook":
      message.channel.send(
        "Find out Facebook page here : https://www.facebook.com/pokemonedmond/"
      );
